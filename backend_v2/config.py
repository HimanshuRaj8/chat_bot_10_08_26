"""
Backend V2 — Configuration

Reads from environment or .env file.
Runs on port 8001 during parallel development.
"""
import os

# ── Directory Layout ─────────────────────────────────────────────────────────
BACKEND_V2_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR       = os.path.dirname(BACKEND_V2_DIR)
...
# ── Excel Data Files ──────────────────────────────────────────────────────────
# Prefer uploaded (latest) files; fall back to Sample folder
...
# ── Server ────────────────────────────────────────────────────────────────────
FLASK_PORT  = int(os.environ.get("FLASK_PORT_V2", 8001))
SECRET_KEY  = os.environ.get("SECRET_KEY", "motherson-approval-system-v2-2026")

# ── Data Provider ─────────────────────────────────────────────────────────────
DATA_PROVIDER_TYPE = os.environ.get("DATA_PROVIDER_TYPE", "excel")

# ── Microsoft Entra ID ────────────────────────────────────────────────────────
ENTRA_TENANT_ID  = os.environ.get("ENTRA_TENANT_ID",  "common")
ENTRA_CLIENT_ID  = os.environ.get("ENTRA_CLIENT_ID",  "motherson-approval-app-client-id")

# ── Chat History ──────────────────────────────────────────────────────────────
# Shared with old backend so history is preserved after cutover
