"""
Backend V2 — Date Parser Utility

Converts natural language date phrases to (start, end) datetime tuples.
This is deterministic and never calls the LLM.
"""
import re
import logging
...
def parse_date_phrase(text: str) -> DateRangeTuple:
    """
    Detects date-related phrases in text and returns (start, end, label).
    Returns (None, None, "") if no date phrase found.
    """
    ...
    for i, (full, abbr) in enumerate(zip(month_names, month_abbrs), start=1):
        # e.g. "march 2025", "march", "mar", "mar 2025"
