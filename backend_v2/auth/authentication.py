"""
Backend V2 — AuthService

Authenticates users by email (simulating Microsoft Entra ID).
...
class AuthService:
    ...
    def __init__(self, data_provider: DataProvider, session_store: SessionStore):
    ...
    def authenticate_email(self, email: str) -> Tuple[bool, Optional[CurrentUser], str]:
        ...
        # Generate deterministic session token
    ...
    def verify_entra_token(self, token: str) -> Tuple[bool, Optional[CurrentUser], str]:
        """
        Verifies a Microsoft Entra ID JWT (unverified decode for email extraction).
        Production: replace with proper public key verification.
        """
        try:
            # Optional import — PyJWT may not be present in some environments
            ...
            logger.warning("PyJWT not installed — Entra token verification not available.")
    ...
    def get_user_from_session(self, session_token: str) -> Optional[CurrentUser]:
        """Returns the CurrentUser associated with a valid session token."""
